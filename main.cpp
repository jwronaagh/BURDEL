...
/* Co dodać?
* licznik czasu// w sobotę z tym podziałam
...
int main()
{
    ...
    resetButton.setPosition(400, 10); // Ustawiam pozycję przycisku na ekranie
    ...
    wygrana.setPosition(150, 400);
    //Napis timer
    RectangleShape Timer(Vector2f(100, 50));
    Clock clock;
    Text timerText;
    timerText.setFont(open_sans);
    timerText.setCharacterSize(24);
    timerText.setPosition(350, 100);
    ...
    while (window.isOpen())
    {
        ...
        {
            // Pobieramy pozycję kursora myszy
            Vector2i mousePos = Mouse::getPosition(window);

            // Sprawdzamy, czy kursor jest nad kwadratem
            if (mousePos.x > 0 && mousePos.x < 500 && mousePos.y > 0 && mousePos.y < 500)
            {
                // Obliczamy indeks klikniętego kwadratu
                int x = mousePos.x / w;
                int y = mousePos.y / w;

                // Odkrywamy puste pola
                odkryjPuste(x, y);
            }
        }
        if (sgrid[x][y] == 9)
        ...
        // wizualizowanie zawartości
        window.draw(resetButton);
        window.draw(reset);
        window.display();
       
        Time elapsed = clock.getElapsedTime();
        string timerString = "Czas: " + std::to_string(elapsed.asSeconds());
